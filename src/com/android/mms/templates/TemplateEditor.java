...
public class TemplateEditor extends Activity {
    ...
    private static final String LOG_TAG = TemplateEditor.class
	    .getCanonicalName();
	...
	Log.i(LOG_TAG, "gesture is: "
		+ (gestures == null ? "null" : "not null"));
	...
	db.open();
	final String messageText = db
		.getTemplateTextFromId(mCurrentTemplateEditingId);
	db.close();
	...
	mGesture = loadGestureIfExists(String
		.valueOf(mCurrentTemplateEditingId));
	if (mGesture != null) {
	    overlay.post(new Runnable() {
		public void run() {
		    overlay.setGesture(mGesture);
	...
	if (bundle != null) {
	    final int displayType = bundle.getInt(KEY_DISPLAY_TYPE,
		    DISPLAY_TYPE_NEW_TEMPLATE);

	    if (displayType == DISPLAY_TYPE_EDIT_TEMPLATE) {
		editingMode = true;

		mCurrentTemplateEditingId = bundle.getLong(KEY_TEMPLATE_ID,
			Long.MIN_VALUE);
	...
	if (templateText.trim().length() == 0) {
	    Toast.makeText(this, R.string.template_empty_text,
